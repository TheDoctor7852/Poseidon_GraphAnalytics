...
std::vector<relationship*> analytics::dfs(graph_db_ptr& graph, node::id_t start){
  utils::PropertyTracker<bool> found = utils::PropertyTracker<bool>(graph->get_nodes()->as_vec().capacity(), false);
  std::vector<relationship*> partial_vec = {};
  found.propertys[start] = true;
...
*/
void analytics::dfs_recursive (graph_db_ptr& graph, node::id_t start, utils::PropertyTracker<bool>& found, std::vector<relationship*>& partial_vec){
  graph->foreach_from_relationship_of_node(graph->node_by_id(start),[&] (relationship& r) {
        node::id_t to_node = r.to_node_id();
        if((!found.propertys[to_node])){
            found.propertys[to_node] = true;
...
/*
  ...
  Rückgabe ist hierbei ein Vector der alle durchlaufenen Kanten enthält, welche sich hierbei im partial_vec befinden.
*/
void analytics::dfs_recursive (graph_db_ptr& graph, node::id_t start, utils::PropertyTracker<bool>& found, std::vector<relationship*>& partial_vec,std::vector<node::id_t>& end, bool& found_endnode){
  if(found_endnode){
    return;
  } else{
    graph->foreach_from_relationship_of_node(graph->node_by_id(start),[&] (relationship& r) {
      node::id_t to_node = r.to_node_id();
       if((!found.propertys[to_node]) && (!found_endnode)){
        found.propertys[to_node] = true;
        partial_vec.push_back(&r);
